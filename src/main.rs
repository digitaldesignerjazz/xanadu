...
struct State {
    ...
    kv: Kv,
    assembler: Assembler,
    last_snap_push: HashMap<String, Instant>,
}
...
async fn main() -> Result<()> {
    ...
    let state = Arc::new(Mutex::new(State {
        ...
        last_snap_push: HashMap::new(),
    }));

    info!(%name, %node_id, listen = %args.listen, cluster = ?args.cluster, "xanadu v0.5 starting");
        ...
        if line == "/snapshot" {
            let ok = {
                ...
                if st.raft.role != Role::Leader {
                    warn!("snapshot skipped: not leader");
                    false
                } else {
                    st.kv.snapshot();
                    let idx = st.kv.last_applied.max(st.raft.commit_index);
                    if idx == 0 {
                        warn!("snapshot skipped: nothing applied");
                        false
                    } else {
                        let term = st.raft.term_at(idx).unwrap_or(st.raft.current_term);
                        st.raft.compact(idx, term);
                        true
                    }
                }
            };
...
async fn push_snapshot(state: &Arc<Mutex<State>>, to: Option<String>) {
    {
        ...
        let key = to.clone().unwrap_or_else(|| "*".into());
        let now = Instant::now();
        if let Some(prev) = st.last_snap_push.get(&key) {
            if now.duration_since(*prev) < Duration::from_millis(400) {
                return;
            }
        }
        st.last_snap_push.insert(key, now);
    }
    let frames = {
        let st = state.lock().await;
        if st.raft.role != Role::Leader {
        ...
        let blob = SnapBlob {
            last_included_index: st
                .raft
                .last_included_index
                .max(st.kv.last_applied.max(st.raft.commit_index)),
            last_included_term: if st.raft.last_included_index > 0 {
                st.raft.last_included_term
            } else {
                st.raft.current_term
            },
            map: st.kv.map_clone(),
        };
        ...
        for (offset, part, done) in parts {
            let hex = to_hex(&part);
            frames.push(Wire::Raft {
                rpc: Rpc::InstallSnapshot {
                            ...
                            if term < st.raft.current_term {
                                ...
                                match st.assembler.push(
                                    ...
                                    Ok(Some(blob)) => {
                                        ...
                                        st.raft.install_snapshot(
                                            blob.last_included_index,
                                            blob.last_included_term,
                                        );
                                        info!(
                                            from = %leader_id,
...
async fn dispatch(state: &Arc<Mutex<State>>, actions: Vec<Action>) {
    for action in actions {
        match action {
            Action::Info(msg) => {
                info!(raft = %msg, "raft");
                if let Some(peer) = msg.strip_prefix("need-snapshot ") {
                    let peer = peer.trim().to_string();
                    if !peer.is_empty() {
                        push_snapshot(state, Some(peer)).await;
                    }
                }
            }
